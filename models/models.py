...
class Base(DeclarativeBase):
    id: Mapped[UUID] = mapped_column(
        primary_key=True, unique=True, type_=UUID(as_uuid=False)
    )
...
class User(Base):
    ...
    password: Mapped[str]
    receive_email: Mapped[bool] = mapped_column(default=True)
...
class SocialAccount(Base):
    ...
    type: Mapped[bool]
    last_scanned: Mapped[datetime] = mapped_column(default=func.now())
